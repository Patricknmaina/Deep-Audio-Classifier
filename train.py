...
class DataGenerator(tf.keras.utils.Sequence): # Creates a custom data generator
    ...
    # Calculate the number of batches in data generator
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        for i, (path, label) in enumerate(zip(wav_paths, labels)):
            ...
            X[i,] = wav.reshape(-1, 1)
            Y[i,] = to_categorical(label, num_classes=self.n_classes) # Hot-encoded matrix

        return X, Y
    
    # Shuffle the indexes after one epoch
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Audio Classification Model Training')
    ...
    parser.add_argument('--delta_time', '-dt', type=float, default=1.0,
                        help='audio sample time in seconds')
